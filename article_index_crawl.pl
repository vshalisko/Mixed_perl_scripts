...
##      Version that crawls subdirectories from start directory
##---------------------------------------------------------------------------##
##    
...
my (%anchtor,%description,%link,%lang,%linklist);

chdir $dir_start || die "\nCan`t open directory $dir_start\n";
...
sub crawl {
   ...
   opendir(DIR, $crawl_dir) or (warn "\nCannot open $crawl_dir: $!" and next);
    my @dircontents = readdir DIR; 
   closedir(DIR);
   my @subdirs = grep {-d and not ( /^\.{1,2}$/ or /^.*\.files$/i or /^.*_archivos$/i )} @dircontents;
  
   foreach my $subdir (@subdirs) {
     my $full_subdir = $crawl_dir."\\".$subdir;
     chdir $full_subdir || die "\nCan`t go to directory $full_subdir\n";
     &crawl($full_subdir); 
   };

  chdir $crawl_dir || die "\nCan`t open directory $crawl_dir\n";
  if (-e $outfile) { 
  ...
  (%anchtor,%description,%link,%lang,%linklist) = (); 

  &SCANFILES($crawl_dir);
  &CREATEINDEX($crawl_dir);
