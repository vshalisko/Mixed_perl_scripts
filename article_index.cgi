...
my $root = "/mnt/external_drive/Library";

my $dir = param('dir') || $root;

my (%anchtor,%description,%link,%lang);

chdir $dir || warn "Can`t open directory $dir\n";
...
sub SCANFILES {
   ...
   my @indexfilemask = ("index.ht*");

   chdir $dir || warn "Can`t go to directory $dir\n";
   ...
   foreach my $subdir (@subdirs) {
     chdir $subdir || warn "Can`t go to directory $subdir\n";
    ...
    $anchtor{$path_with_slash} = "indexed&nbsp;directory&nbsp;<a href='$dir\\$path'><b>$dir\\$path<\/b><\/a>";
     };
      chdir $dir || warn "Can`t go to directory $dir\n";
   };
   ...
   while(<@textfilemask>) {
    ...
    if ($description{$_} =~ /(L|l)=(ru|sp|en|de|fr|la|gr)/) {
      ...
      $description{$_} =~ s/(L|l)=(ru|sp|en|de|fr|la|gr)//;
           ...
           $anchtor{$_} = "text&nbsp;file&nbsp;<a href=\"$dir\\$_\"><b>$dir\\$_<\/b><\/a>,&nbsp;$size&nbsp;k";
   };
   ...
   while(<@imagefilemask>) {
     ...
     my $size=sprintf("%.2f", ($inode->size)/1024);
      ...
      $description{$_}=$d->description($_);
           ...
           $anchtor{$_} = "image&nbsp;file&nbsp;<a href=\"$dir\\$_\"><b>$dir\\$_<\/b><\/a>,&nbsp;$size&nbsp;k";
   };
   ...
   ($anchtor{$a} cmp $anchtor{$b}) || ($a cmp $b) } keys %anchtor) {
     ...
     if ($link{$anchtorpath}) {
      ...
      $i++;
            ...
            print "&nbsp;<A HREF=#from_".$item."><SUP>".$item."</SUP></A>";
     };
     ...
     print "\n";
      ...
      print "<LI TYPE=1 VALUE=".$j."><A NAME=from_".$j."></A>from ".$linklist{$j}."\n";
       };
      ...
      print "<BR><ADDRESS>Y".(localtime->year()+1900).", \u$host, "; 
