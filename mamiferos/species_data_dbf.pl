...
## already is in this directory programm will not work correctly
##
## How to use: perl species_data_dbf.pl datafile
##
## Database format:
...
foreach my $id (sort {$generos{$a} cmp $generos{$b}
	...
	if ($specie_previous ne $species{$id}) {
		...
		$outfile = $generos{$id}."_".$species{$id}.".dbf";
		&dbf_create($outfile);
		&dbf_line_output($outfile,$database{$id});	
	} else {
		print ".$id";	
		&dbf_line_output($outfile,$database{$id});
	}
...
print "\n\nEnd of work. Database processed.";
...
#	$dbf = new XBase $_[0] or die XBase->errstr;
	$dbf = XBase->create("name" => $_[0],
                "field_names" => [  "IdReg", "Genero", "Especie", "Latitude", "Longitude", "Origen" ],
